...
async function buildServerCheckout(db, requestCart, customerName) {
  ...
  for (const product of products) {
    ...
    const unitPrice = promotionIsActive
      ? discountPrice
      : regularPrice;
  ...
  const isFirstOrder =
    Number(previousOrders[0].order_count) === 0;

  const discountCents = isFirstOrder
...
module.exports = function(db) {
        ...
        try {
            ...
            return res.status(500).json({
                success: false,
                message:
                    "Unable to generate the payment QR."
            });
        }
    });
    ...
    router.post(
        "/payments/khqr/verify",
        async (req, res) => {
            const paymentAttemptId = String(
                req.body.paymentAttemptId || ""
            ).trim();
            ...
            if (!paymentAttemptId) {
                return res.status(400).json({
                    ...
                    verified: false,
                    message:
                        "A payment attempt ID is required."
                });
            }
            ...
            try {
                ...
                if (
                    ...
                    return res.status(400).json({
                        ...
                        expired: true,
                        message:
                            "This KHQR has expired. Generate a new QR."
                    });
                }
                ...
                const attemptsUsed = Number(
                    attempt.verification_attempts || 0
                );
                ...
                if (attemptsUsed >= maxAttempts) {
                    ...
                    return res.status(429).json({
                        ...
                        attemptsRemaining: 0,
                        message:
                            "Maximum verification attempts reached for this QR."
                    });
                }

                if (attempt.last_verification_at) {
                    const lastAttemptAt = new Date(
                        attempt.last_verification_at
                    );
                ...
                console.error(
                    "Unable to prepare KHQR verification:",
                    error
                );
                    ...
                    verification = {
                        ...
                        transaction: {
                            ...
                            currency:
                                attempt.currency,
                            toAccountId:
                                attempt.receiver_account
                        }
                    };
                } else {
                    verification =
                        await verifyTransactionByMd5(
                            attempt.khqr_md5
                        );
                }
            } catch (error) {
            ...
            try {
                ...
                if (
                    lockedAttempt.status !== "Generated"
                ) {
                    await finalConnection.rollback();
                    ...
                    return res.status(409).json({
                        ...
                        verified: false,
                        message:
                            "This payment attempt cannot be completed."
                    });
                }

                const cart = JSON.parse(
                    lockedAttempt.cart_snapshot
                );
                ...
                if (
                    ...
                    throw new Error(
                        "Stored cart snapshot is invalid."
                    );
                }
                ...
                for (const item of cart) {
                    ...
                    if (
                        ...
                        throw new Error(
                            "Stock changed during payment finalization."
                        );
                    }
                }
                ...
                if (error.code === "ER_DUP_ENTRY") {
                    return res.status(409).json({
                        ...
                        verified: false,
                        message:
                            "This Bakong transaction has already been used."
                    });
                }
