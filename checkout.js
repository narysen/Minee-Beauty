...
async function renderCheckoutPage() {
  ...
  try {
    ...
    console.warn(
      "Could not load raw database product variations for checkout comparison display.",
    );
  }

  checkoutCart.forEach((item) => {
    const dbMatch = databaseProductsList.find(
    ...
    let fixedPromotionIsActive = false;
    let bulkPromotionIsActive = false;
    let bulkMinimum = 0;
    let bulkPercent = 0;

    if (dbMatch) {
      const now = new Date();
      const regularPrice = Number(dbMatch.price);

      const discountPrice =
        dbMatch.discount_price === null || dbMatch.discount_price === undefined
          ? null
          : Number(dbMatch.discount_price);

      const startsAt = dbMatch.discount_start
        ? new Date(dbMatch.discount_start)
        : null;

      const endsAt = dbMatch.discount_end
        ? new Date(dbMatch.discount_end)
        : null;

      fixedPromotionIsActive =
      ...
      const promotionalUnitPrice = fixedPromotionIsActive
        ? discountPrice
        : regularPrice;

      bulkMinimum = Number(dbMatch.bulk_min_quantity);

      bulkPercent = Number(dbMatch.bulk_discount_percent);

      bulkPromotionIsActive =
      ...
      const calculatedUnitPrice = bulkPromotionIsActive
      ...
      // round the discounted price for one unit to cents.
      finalItemPrice = Math.round(calculatedUnitPrice * 100) / 100;
    }

    const rowTotalCents = Math.round(finalItemPrice * 100) * quantity;

    const rowTotal = rowTotalCents / 100;

    total += rowTotal;

    const hasAnyDiscount = fixedPromotionIsActive || bulkPromotionIsActive;
    ...
    <div class="flex items-center py-3 px-1 border-b border-neutral-100/60 last:border-0">
      <div class="flex-[2_2_0%] font-medium text-xs truncate max-w-[180px]">
        <p class="text-neutral-800 font-semibold truncate">
          ${item.title}
        </p>
        ...
        ${
          fixedPromotionIsActive
            ? `
              <span class="inline-block text-[10px] bg-red-50 text-red-600 px-1.5 py-0.5 rounded font-bold">
                On Sale
              </span>
            `
            : ""
        ...
        ${
          bulkPromotionIsActive
            ? `
              <span class="inline-block text-[10px] bg-emerald-50 text-emerald-700 px-1.5 py-0.5 rounded font-bold">
                Buy ${bulkMinimum}+, Save ${bulkPercent}%
              </span>
            `
            : ""
        }
      ...
      <div class="flex-1 text-center text-neutral-500 text-xs">
        x${quantity}
      </div>
      ...
      <div class="flex-[1.5_1.5_0%] text-right text-xs">
        ${
          hasAnyDiscount
            ? `
              <span class="text-neutral-400 line-through block text-[10px]">
                $${originalPrice.toFixed(2)}
              </span>
              <span class="text-pink-600 font-bold block">
      ...
      <div class="flex-1 text-right font-black text-neutral-800 text-xs font-mono">
        $${rowTotal.toFixed(2)}
      </div>
    </div>
  `;
  ...
  if (userQueryKey !== "Guest") {
    try {
      ...
      console.warn(
        "Backend connection missed. Defaulting to true for testing first order layout.",
      );
      isFirstOrder = true;
    }
  }
...
function setupAddressUI() {
  ...
  if (
    ...
    user.address !== "No address assigned"
  ) {
...
window.selectPayment = function (method) {
  ...
  document
    .querySelectorAll(".pay-selector-btn")
    .forEach((btn) => btn.classList.remove("border-pink-400", "bg-pink-50"));
  ...
  if (method === "Bank QR") {
    ...
    document
      ...
      .classList.add("border-pink-400", "bg-pink-50");

    const attemptIsActive =
      currentKHQRAttempt &&
      Date.now() < new Date(currentKHQRAttempt.expiresAt).getTime();
...
function getCheckoutCustomerDetails() {
  ...
  let address = String(user.address || "").trim();
  ...
  return {
    customerName: String(user.name || "Guest Customer").trim(),
    phone,
    address,
  };
}
...
function setKHQRView(view) {
  const loading = document.getElementById("khqr-loading");

  const content = document.getElementById("khqr-content");

  const error = document.getElementById("khqr-error");

  loading.classList.toggle("hidden", view !== "loading");

  content.classList.toggle("hidden", view !== "content");

  error.classList.toggle("hidden", view !== "error");
}

function showKHQRError(message) {
  document.getElementById("khqr-error-message").innerText = message;

  setKHQRView("error");
}

function startKHQRExpirationTimer(expiresAt) {
  if (khqrExpirationTimer) {
    clearInterval(khqrExpirationTimer);
  }

  const expirationElement = document.getElementById("khqr-expiration");

  const verifyButton = document.getElementById("verify-khqr-button");
  ...
  const updateTimer = () => {
    ...
    if (remainingMilliseconds <= 0) {
      ...
      expirationElement.innerText = "KHQR expired";

      verifyButton.disabled = true;

      showKHQRError("This KHQR has expired. Generate a new payment QR.");
  ...
  khqrExpirationTimer = setInterval(updateTimer, 1000);
}

window.generateKHQRPayment = async function () {
  if (khqrGenerationRunning) {
    return;
  }

  if (!checkoutCart.length) {
    showKHQRError("Your cart is empty.");
  ...
  try {
    customer = getCheckoutCustomerDetails();
  } catch (error) {
  ...
  try {
    const response = await fetch(`${API_URL}/payments/khqr`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok || !data.success) {
      throw new Error(data.message || "Unable to generate KHQR.");
    }
    ...
    document.getElementById("khqr-status").innerText =
    ...
    startKHQRExpirationTimer(data.expiresAt);
  } catch (error) {
    console.error("KHQR generation error:", error);

    showKHQRError(error.message || "Unable to generate KHQR.");
  } finally {
    khqrGenerationRunning = false;
  }
};
function completeVerifiedKHQROrder(data) {
  const history = JSON.parse(localStorage.getItem("orderHistory")) || [];

  history.push({
    orderId: data.orderId,

    customerName: currentKHQRAttempt.customerName,

    totalPrice: currentKHQRAttempt.amount.toFixed(2),
...
function beginVerificationCooldown(seconds) {
  const button = document.getElementById("verify-khqr-button");
...
window.verifyKHQRPayment = async function () {
  if (khqrVerificationRunning || !currentKHQRAttempt) {
    return;
  }

  const expiresAt = new Date(currentKHQRAttempt.expiresAt).getTime();
  ...
  status.className = "mt-3 text-xs text-blue-600";

  status.innerText = "Checking your transaction with Bakong...";

  try {
    const response = await fetch(`${API_URL}/payments/khqr/verify`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        paymentAttemptId: currentKHQRAttempt.id,
      }),
    });
    ...
    if (data.verified) {
      status.className = "mt-3 text-xs font-semibold text-emerald-600";

      status.innerText = "Payment verified successfully.";

      button.innerHTML =
        '<i class="fa-solid fa-check mr-1"></i> Payment Verified';

      completeVerifiedKHQROrder(data);

      return;
    }

    if (response.status === 429) {
      status.className = "mt-3 text-xs text-amber-600";

      status.innerText = data.message;

      beginVerificationCooldown(data.retryAfter || 60);
    ...
    status.innerText = error.message || "Unable to verify payment.";
...
window.finishAssumedOrder = async function () {
  ...
  if (!document.getElementById("address-form").classList.contains("hidden")) {
    ...
    if (!phoneVal || !addressVal) {
      alert(
        "Please input your Phone Number and Delivery Address before checking out!",
      );
      return;
    }
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(data.message || "The order could not be completed.");
    }
  ...
  orderHistory.push({
    ...
    totalPrice: Number(completedOrderData.total).toFixed(2),
